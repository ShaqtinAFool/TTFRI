...
1. Linux 設定環境變數，如Fig 3

	> vi ~/.bashrc
	> export PATH=/usr/local/bin/anaconda2/bin:$PATH
	> source ~/.bashrc
	> Windows設定環境變數，如Fig 4箭頭順序
...
1. 之後加入 wradlib 所在的軟體庫 conda-forge (如 Fig 6)
...
1. 即會輸出 wradlib 的版本(如 Fig 10, Fig 11)，表示 wradlib 安裝成功。以上為官網的安裝方式，官網另有其他方式可以安裝，有興趣可到官網查看。
...
把 wradlib 和 pyart 裝在同個環境後可一起用，不用再切換。建議再安裝一個 basemap(如 Fig 13)，這是世界地圖的模組。 
...
#	讀檔
## Rainbow5
前文有提到 rainbow5 檔案在檔頭會有一段資料內容的描述，wradlib 是依照這個檔頭的內容去讀取存在檔案內的二進位資料。讀檔前首先要載入需要的模組(Fig 14)，然後選擇要讀的檔案。之後用 wradlib 中針對 rainbow5 格式的功能進行讀檔(Fig 15)。
